...
app.use((req, res, next) => {
  ...
  const isBot =
    /googlebot|bingbot|yandex|baiduspider|twitterbot|facebookexternalhit|rogerbot|linkedinbot|embedly|quora\slink\spreview|showyoubot|outbrain|pinterest\/0\.|pinterestbot|slackbot|vkShare|W3C_Validator|lighthouse|chrome-lighthouse/i.test(
      userAgent,
    );
  ...
  res.locals.seo = {
    ...
    description:
      "Ankit Singh, a Full Stack Developer specializing in React, MERN Stack developer, Node.js & MongoDB. View projects, skills, and blog.",
    keywords:
      "Ankit Singh, Full Stack Developer, React Developer, Node.js, Portfolio, Blog",
  ...
  try {
    ...
    res.render("index", {
      ...
      seo: {
        ...
        description:
          "Ankit Singh is a Full Stack Developer skilled in React.js, Node.js, JavaScript, MongoDB, Express.js and modern web development. Explore projects, blogs and software engineering expertise.",
        keywords:
          "Ankit Singh, Full Stack Developer, React.js Developer, Node.js Developer, MERN Stack Developer, JavaScript Developer, Software Engineer, MongoDB Developer, Express.js Developer",
        image: "https://codewithaks.in/images/ankit-singh.webp",
  ...
  try {
    ...
    res.render("blogs", {
      ...
      seo: {
        ...
        description:
          "Read expert guides on Artificial Intelligence, Full Stack Development, React.js, Node.js, SEO and career growth by Ankit Singh – Full Stack Developer.",
        keywords:
          "tech blog, web development tutorials, AI blog, SEO tips, Node.js tutorials, React guides",
        image: "https://codewithaks.in/images/ankit-singh.webp",
...
app.get("/blog/:slug", async (req, res) => {
  try {
    ...
    const [recentBlogs, categoryBlogs, randomBlogs] = await Promise.all([
      Blog.find({ status: "Published", slug: { $ne: req.params.slug } })
        .sort({ createdAt: -1 })
        .limit(5),
      Blog.find({
        status: "Published",
        ...
        .limit(6),
      Blog.aggregate([
        { $match: { status: "Published", slug: { $ne: req.params.slug } } },
        { $sample: { size: 3 } },
      ]),
    ]);
    ...
    let faqData = [];
    try {
      faqData = JSON.parse(blog.faqSchema || "[]");
    } catch (e) {}
    ...
    res.render("single_blog", {
      ...
      seo: {
        ...
        image: blog.image || "https://codewithaks.in/images/ankit-singh.webp",
...
const adminNotifications = async (req, res, next) => {
  try {
    res.locals.unreadContactsCount = await Contact.countDocuments({
      isRead: false,
    });
  } catch (err) {
...
app.post(
  ...
  async (req, res) => {
    const renderView = (error, success = null) =>
      res.render("dhanrubi/change-password", {
        error,
        success,
        seo: {
    ...
    if (newPassword !== confirmPassword)
      return renderView("New passwords do not match.");
...
app.use("/dhanrubi/legacy", (req, res) =>
...
app.post(
  "/dhanrubi/blog/toggle-status/:id",
  requireAdmin,
  csrfProtection,
  async (req, res) => {
    try {
...
app.get("/api/dhanrubi/faqs", requireAdmin, async (req, res) => {
  try {
    ...
    if (req.query.search)
      q.question = { $regex: req.query.search, $options: "i" };
...
// POST add FAQ
app.post(
  "/api/dhanrubi/faq/add",
  requireAdmin,
  csrfProtection,
  async (req, res) => {
    try {
      let { category, question, answer, status, slug, tags, breadcrumbSchema } =
        req.body;
  ...
  async (req, res) => {
    try {
      ...
      if (slug && slug !== faq.slug) {
        ...
        if (dup)
          return res.json({ success: false, message: "Slug already in use." });
      }
  ...
  try {
    ...
    const [today, yesterday, week, total] = await Promise.all([
      ...
      PageView.countDocuments({
        visitedAt: { $gte: yesterdayStart, $lt: todayStart },
      }),
...
app.get("/api/dhanrubi/analytics/log", requireAdmin, async (req, res) => {
  try {
    ...
    if (req.query.page)
      match.pageUrl = { $regex: req.query.page, $options: "i" };
    const data = await PageView.find(match)
      .sort({ visitedAt: -1 })
      .limit(500)
      .lean();
    res.json({ success: true, data });
